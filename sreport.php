...
$sql="SELECT invno,`acname`,`date`,`gstin`,sum(`Amount`),sum(`cgst`),sum(`sgst`),sum(`igst`),sum(`namount`) FROM `sales` GROUP by invno";
...
echo '<tr> <td>'.$row['invno'].'</th><th>'.$row['date'].'<th><a href="sprint.php?inv='.$row['invno'].'">'.$row['acname'].'</a><th>'.$row['gstin'].'<th>'.$row['sum(`Amount`)'].'<th>'.$row['sum(`cgst`)'].'<th>'.$row['sum(`sgst`)'].'<th>'.$row['sum(`igst`)'].'<th>'.$row['sum(`namount`)'];
